...
export default function Testimonials() {
  ...
  const calculateBoxHeight = () => {
    ...
    visibleTestimonials.forEach((testimonial, index) => {
      ...
      const boxHeight = element ? element.offsetHeight : 0;
      columnHeights[columnIndex] += boxHeight;
    });
              ...
              {visibleTestimonials.map(
                (testimonial, index) =>
                  index % columns === columnIndex && (
                    ...
                    className="group relative bg-[#FFF3EA] hover:bg-[#FFFFFF] px-8 py-10 rounded-2xl space-y-5"
                    style={{ height: calculateBoxHeight(), width: `${100 / columns}%` }}
                  >
