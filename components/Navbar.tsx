...
const NavBar = () => {
  ...
  return (
    <header className="absolute top-0 w-full px-4 py-4 z-[5] ">
      <nav className="w-full flex items-center justify-between max-w-[1420px] mx-auto">
        <Logo />
        <div className="w-10 grid place-content-center ml-6 lg:hidden">
          <div
            ...
            <span
              className={`w-4 relative h-[2px] bg-[#c4c8d4] transition  rounded-full duration-100 opacity-100 ${isExpanded && "opacity-0 invisible"} `}></span>
            <span
        ...
        <div className="hidden lg:flex justify-end space-x-4">
          <motion.button
            className="text-white flex flex-row items-center justify-center bg-transparent border border-white rounded-lg px-4 py-2 outline-none cursor-pointer"
            whileHover={{ scale: 1.2 }}>
            <AiOutlineGlobal className="mr-2 item-center" />
            English
          </motion.button>
          <motion.button
            className="text-white flex flex-row items-center justify-center bg-orange-500 hover:bg-orange-600 rounded-lg px-4 py-2"
            whileHover={{ scale: 1.2 }}>
            Sign Up
